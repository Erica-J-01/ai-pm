![GitHub stars](https://img.shields.io/github/stars/Erica-J-01/ai-pm-skills-)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow?style=flat-square)](https://github.com/Erica-J-01/ai-pm-skills-/blob/main/LICENSE)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen?style=flat-square)](https://github.com/Erica-J-01/ai-pm-skills-/pulls)
...
Each skill encodes a proven PM workflow — intake triage, risk analysis, discovery, PRDs, user stories, sprint planning — and walks Claude through it step by step. You get the rigour of a senior PM built into your terminal.
...
```bash
git clone https://github.com/Erica-J-01/ai-pm-skills-.git
cd ai-pm-skills-
claude .
```

Claude reads `CLAUDE.md` and `.claude/CLAUDE.md` automatically, and registers slash commands from `.claude/commands/`. No configuration is needed to start.

---

## Connecting Your Tools (MCP) - optional

Everything works out of the box as a text-in, markdown-out tool. Connecting your tools is optional. It removes copy-paste: skills can then read live Jira data and publish straight to Confluence, Google Drive, Notion, or Gmail.
...
1. **Claude connectors (simplest).** Enable the Atlassian (Jira/Confluence), Google Drive, Notion, and Gmail connectors in Claude. The skills are pre-wired to the resulting tool names. No config files are needed.
...
2. **Self-hosted MCP servers.** Copy the template and fill in your details:
   ...
   Get an Atlassian API token from `id.atlassian.com` under Security, then API tokens.

> Personal overrides and secrets belong in `.claude/settings.local.json`, `.env`, or `.mcp.json`. All are gitignored. Never hardcode tokens in tracked files.
...
**When to use:**  
- You receive a raw message and need to understand what is actually being asked  
...
```
/triage
Here's a message from our client: "We need the reporting module to show 
...
- Synthesising notes from a workshop you have already run  
...
**What it does:**  
Runs a structured 10-question intake interview before writing. It asks one question at a time to confirm document type (PRD vs BRD), scope boundaries, target users, success metrics, constraints, and NFR targets. Conditional questions trigger automatically when signals like stale input, regulated context, or missing out-of-scope items are detected. No document is generated until the final gate question is answered.

Once intake is complete it produces a full PRD or BRD: purpose and background, goals and success metrics, users and stakeholders, assumptions and constraints, functional requirements (grouped by feature area with error states as first-class FRs), non-functional requirements (with concrete targets), out-of-scope items, dependencies, open questions, and a sign-off table. Any scope divergence from the source document is logged in a dedicated Scope Changes section.
...
- Aligning stakeholders on what will and will not ship
...
<details>
<summary><strong>meeting-notes</strong> - Clean meeting minutes from raw transcripts</summary>
...
- Sharing outcomes with stakeholders who were not in the room
...
- Aligning stakeholders on what will and will not ship this sprint
...
Goal: Ship the payments feature with live data.
Backlog: PROJ-12 Payments UI (3pts), PROJ-13 Data API (5pts), PROJ-14 Export (2pts), PROJ-15 Notifications (3pts - stretch).
...
- When a stakeholder asks whether you are ready to ship
...
**What it does:**  
Facilitates a retro (format, agenda, prompts) or turns raw retro notes into a structured summary: what went well, what did not (as blameless themes), and action items each with a single owner and a date. Reviews whether the last retro's actions actually got done.
...
**What it does:**  
Translates delivery detail into a concise update for a specific audience (sponsor, exec, client, or team): RAG status, progress as outcomes, what is next, risks, and any decisions needed. Drafts it as an email, Confluence page, or copy-ready text.
...
**What it does:**  
Sequences initiatives into themes and time horizons with dependencies and honest confidence levels (near-term firm, later directional). Produces a Now/Next/Later or quarterly view, plus what is explicitly parked and why.
...
**What it does:**  
Compares spend to date against the charter budget, forecasts cost at completion, and flags overrun early with a RAG verdict, variance drivers, and recommended actions. Built to catch problems while there is still time to act.

**When to use:**  
- Monthly or per-sprint budget check-ins  
- Answering whether you are on budget  
...
.claude/
  ...
  settings.json                      # Permissions config (scoped to clients/)
skills/                              # Skill definitions (project root)
  ...
  retrospective/  stakeholder-update/  roadmap/                  # standalone skills
  budget-tracker/
    SKILL.md                         # each skill: SKILL.md (plus reference.md where helpful)
...
3. Follow the existing skill structure (`SKILL.md` plus `reference.md` minimum)
...
- Output template (the exact structure Claude will follow)
- Save prompt following the Saving Artefacts rules in `.claude/CLAUDE.md`
- A `reference.md` worked example (input to output) where helpful
...
Created by **Erica** ([Erica-J-01](https://github.com/Erica-J-01)).
