...
- [x] **No code-splitting - one 1.43 MB entry chunk** (428 kB gzip), three times Vite's warning. The pattern is already proven since pdfjs is split. Add `React.lazy` and `manualChunks`. Sub-wins, biggest first: - DONE: entry chunk now ~257 kB (79 kB gzip), well under the size warning. Later passes removed @tanstack/react-query from the critical path, lazy-loaded all artifact views, and code-split the demo seed corpus (portalData/acmeData/sampleArtifacts) out of the entry chunk.
- [x] **framer-motion on the critical path - DELIBERATELY KEPT.** 114 kB (38 kB gzip), ~19% of the ~195 kB gzip first load, used by four shell components for fades/slides/AnimatePresence exits. Decision (2026-07-11): keep it. The saving is imperceptible on the connections this dashboard targets, exit animations do not map cleanly to CSS, and the visual-regression risk outweighs ~0.1 s. Revisit only if mobile or low-bandwidth users become a target. Do not re-flag in future optimization audits.
